...
#define CACHE_WORD_TAG(cp, addr) ((CACHE_TAG(cp, addr)<<(cp)->set_shift) | CACHE_BLK(cp, addr))
...
unsigned int distill_cache = FALSE;
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  if(distill_cache == TRUE)
  {
			cp->distill_cache = TRUE;
			cp->eviction_counter = 0;
			cp->median_threshold = bsize/4;
			cp->word_eviction_counter = (unsigned int *)calloc(bsize/4, sizeof(unsigned int));
			printf("median threshold: %d\n",cp->median_threshold);
			distill_cache = FALSE;
	}
	else
	{
		cp->distill_cache = FALSE;
	}
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  if (cp->hsize)
		...
		if((cp->distill_cache == TRUE) && (assoc>1 || assoc<8) && (j==(assoc-1))){
			blk->word = (struct cache_word_blk_t *)calloc((bsize/4), sizeof(struct cache_word_blk_t));
			cp->sets[i].way_woc = blk;			
		}
		else
		{	 
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  sprintf(buf1, "%s.hits + %s.woc_hits + %s.misses", name, name, name);
  ...
  sprintf(buf, "%s.woc_hits", name);
  stat_reg_counter(sdb, buf, "total number of hits in word cache", &cp->woc_hits, 0, NULL);
  
  sprintf(buf, "%s.hole_misses", name);
  stat_reg_counter(sdb, buf, "total number of hole misses", &cp->hole_misses, 0, NULL);
  
  sprintf(buf, "%s.line_misses", name);
  stat_reg_counter(sdb, buf, "total number of line misses", &cp->line_misses, 0, NULL);
...
cache_stats(struct cache_t *cp,		/* cache instance */
	    FILE *stream)		/* output stream */
{
  double sum = (double)(cp->hits + cp->misses + cp->woc_hits);

  fprintf(stream,
	  "cache: %s: %.0f hits %.0f word cache hits  %.0f hole misses %.0f line misses %.0f misses %.0f repls %.0f invalidations\n",
	  cp->name, (double)cp->hits, (double)cp->woc_hits, (double)cp->hole_misses,(double)cp->line_misses,(double)cp->misses,
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  md_addr_t word_tag = CACHE_WORD_TAG(cp, addr);
  md_addr_t set = CACHE_SET(cp, addr);
  md_addr_t bofs = CACHE_BLK(cp, addr);
  unsigned int loc_cache_stat = LOC_MISS;
  unsigned int woc_cache_stat = LINE_MISS;
  struct cache_blk_t *blk, *repl, *repl_woc;
  int lat = 0;
  unsigned int repl_woc_line = TRUE;
	char check;
	int line_hit_head_loc = -1;
...
/*	printf("\n Address = 0x%016Lx",addr);
  printf("\n Tag = 0x%016Lx",tag);
  printf("\n Word Tag = 0x%016Lx",word_tag);
  */ 
  ...
  if (CACHE_TAGSET(cp, addr) == cp->last_tagset)
   {
     ...
     /* printf("\n LOC HIT");
			printf("\n Word offset = %d",(bofs/4));
			printf("\n Word use at direct loc %d",CACHE_SET(cp, addr));
		  printf(" is: %d", blk->word_use); 
		  */
			goto cache_fast_hit;		
 	 }
    ...
    for (blk=cp->sets[set].way_head; blk; blk=blk->way_next)
		{
			if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
			{
				//Word is 32 bit, hence each word has 4 bytes
				blk->word_use = blk->word_use | (1<<bofs/4);
			/*	printf("\n LOC HIT");
				printf("\n Word offset = %d",(bofs/4));
				printf("\n Word use at direct loc %d",CACHE_SET(cp, addr));
				printf(" is: %d", blk->word_use); 
				scanf("%c", &check); 
				*/ 
				loc_cache_stat = LOC_HIT;
				goto cache_hit;
			}
		}
		if(cp->distill_cache == TRUE)
		{
			int w_off;
			blk=cp->sets[set].way_woc;
			if (blk->status & CACHE_BLK_VALID)
			{
				for(w_off = 0; w_off<(cp->nwords); w_off++)
				{
					if(blk->word[w_off].tag>>cp->set_shift == tag) 
					{
						woc_cache_stat = LINE_HIT;
					//	printf("\n LINE HIT");
						if(blk->word[w_off].head_bit == 1 && 
					    (blk->word[w_off].status & CACHE_WORD_VALID))
						{
							line_hit_head_loc = w_off;
						}
					//	scanf("%c", &check); 
						if((blk->word[w_off].tag == word_tag) && 
					    (blk->word[w_off].status & CACHE_WORD_VALID))
						{
							/* **HIT** */
							cp->woc_hits++;
					//		printf("\n WORD HIT");
							woc_cache_stat = WORD_HIT;
							/* copy data out of cache block, if block exists */
							if (cp->balloc)
							{
								CACHE_BCOPY(cmd, blk, bofs, p, 4);
							}
							/* update dirty status */
							if (cmd == Write)
								blk->word[w_off].status |= CACHE_WORD_DIRTY;
							
							if(blk->word[w_off].head_bit == 1) w_off++;
...
//	printf("\n LOC STATUS = %d", loc_cache_stat);
//	printf("\n WOC STATUS = %d", woc_cache_stat);
  ...
  if (repl->status & CACHE_BLK_VALID)
    {
      ...
      cp->bus_free = MAX(cp->bus_free, (now + lat)) + 1;
			...
			if(loc_cache_stat == LOC_MISS && woc_cache_stat == WORD_MISS && cp->distill_cache == TRUE)
			{
				int w_off;
				int word_dirty = 0;
				int next_head_bit;
				repl_woc = cp->sets[set].way_woc;
				for(w_off = line_hit_head_loc, next_head_bit = 0; 
						next_head_bit!= 1 && (w_off < cp->nwords); w_off++)
				{
					if(repl_woc->word[w_off].status & CACHE_WORD_DIRTY)
						word_dirty = 1;
					...
					{					
						...
						lat += cp->blk_access_fn(Write,
									CACHE_MK_BADDR(cp, (repl_woc->word[w_off].tag >> (cp)->set_shift), set),
									cp->bsize, repl_woc, now+lat);
					}
					repl_woc->word[w_off].status = 0;
					repl_woc->word[w_off].head_bit = 0;
					if(w_off<(cp->nwords-1))
						next_head_bit = repl_woc->word[w_off+1].head_bit;
				}
			}
      else if (repl->status & CACHE_BLK_DIRTY)
			{
				/* write back the cache block */
				cp->writebacks++;
				lat += cp->blk_access_fn(Write,
	...
	if(cp->threshold_distill_enabled && repl->status & CACHE_BLK_VALID)
	{
		int i;
		int	words_used = 0;		
		...
		cp->eviction_counter++;
		//printf("eviction counter: %d\n",cp->eviction_counter);	
		
		for(i = 0; i<cp->nwords; i++)
			if((repl->word_use>>i) & 1)	words_used++;
		
		if(words_used > cp->median_threshold)
			repl_woc_line = FALSE;	
		else
			repl_woc_line = TRUE;
		
		if(words_used){
			cp->word_eviction_counter[words_used-1]++;
		}
		
		if(cp->eviction_counter % 4096 == 0)
		{
			int eviction_sum = 0;
	//		printf("Reached 4096 evictions\n");
			if(cp->eviction_counter!=0)
			{
				for(i = 0; i<cp->nwords; i++){
	//				printf("Word eviction counter: %d\n",cp->word_eviction_counter[i]);
					}
			}
			
			for(i = 0; i<cp->nwords; i++){
				eviction_sum = eviction_sum + cp->word_eviction_counter[i];
				if(eviction_sum >= cp->eviction_counter/2){
					cp->median_threshold = (i+1);
			//		printf("Median Threshold: %d\n",cp->median_threshold);
					break;
				}
				else
				{
					cp->median_threshold = cp->nwords;
				}
			}
			
			for(i = 0; i<cp->nwords; i++) cp->word_eviction_counter[i] = 0;
			cp->eviction_counter = 0;
		}
    ...
    if(loc_cache_stat == LOC_MISS && 
			 (woc_cache_stat == LINE_MISS || woc_cache_stat == WORD_MISS) 
			  && cp->distill_cache == TRUE && repl_woc_line == TRUE)
			{
				int w_off, i;
				int word_dirty = 0;
				int repl_index = 0;
				int set_head_bit = 0;
				md_addr_t repl_word_tags[cp->nwords];
				unsigned int occ_head_bit_1_loc[cp->nwords];
				unsigned int occ_head_bit_2_loc[cp->nwords/2];
				unsigned int occ_head_bit_4_loc[cp->nwords/4];
				unsigned int occ_head_bit_8_loc[cp->nwords/8];
				unsigned int occ_head_bit_1_count = 0;
				unsigned int occ_head_bit_2_count = 0;
				unsigned int occ_head_bit_4_count = 0;
				unsigned int occ_head_bit_8_count = 0;
				
				unsigned int free_head_bit_1_loc[cp->nwords];
				unsigned int free_head_bit_2_loc[cp->nwords/2];
				unsigned int free_head_bit_4_loc[cp->nwords/4];
				unsigned int free_head_bit_8_loc[cp->nwords/8];
				unsigned int free_head_bit_1_count = 0;
				unsigned int free_head_bit_2_count = 0;
				unsigned int free_head_bit_4_count = 0;
				unsigned int free_head_bit_8_count = 0;
				
				unsigned int repl_word_count = 0;
				
				if(woc_cache_stat == WORD_MISS)	cp->hole_misses++;
				else if(woc_cache_stat == LINE_MISS) cp->line_misses++;
				
				repl_woc = cp->sets[set].way_woc;	
		/*		printf("\n Word use at direct loc %d",CACHE_SET(cp, addr));
				printf(" is: %d",repl->word_use); 
				printf("\n No of words %d",cp->nwords);
				*/ 
				for(w_off = 0; w_off<(cp->nwords); w_off++)
				{

					if((repl->word_use >> w_off) & 1)
					{
						repl_word_tags[repl_word_count++] = (repl->tag << (cp)->set_shift) | (w_off*4); 
				//		printf("\nRepl Word Tag = 0x%016Lx",repl_word_tags[repl_word_count - 1]);
				//		scanf("%c",&check);
					}
					
					if(repl_woc->word[w_off].head_bit == 1 || repl_woc->word[w_off].status != 0)
					{
						occ_head_bit_1_loc[occ_head_bit_1_count++] = w_off;
					}
					else if(repl_woc->word[w_off].status == 0)
						free_head_bit_1_loc[free_head_bit_1_count++] = w_off;
						
					if(w_off % 2 == 0)
					{	
						if(repl_woc->word[w_off].status != 0 || repl_woc->word[w_off+1].status != 0)
								occ_head_bit_2_loc[occ_head_bit_2_count++] = w_off;						
						else if(repl_woc->word[w_off].status == 0)
							free_head_bit_2_loc[free_head_bit_2_count++] = w_off;
					}
					if(w_off % 4 == 0)
					{
						if(repl_woc->word[w_off].status != 0 || repl_woc->word[w_off+1].status != 0 ||
							 repl_woc->word[w_off+2].status != 0 || repl_woc->word[w_off+3].status != 0)
							occ_head_bit_4_loc[occ_head_bit_4_count++] = w_off;
						else if(repl_woc->word[w_off].status == 0)
							free_head_bit_4_loc[free_head_bit_4_count++] = w_off;
					}
					if(w_off % 8 == 0)
					{
						if(repl_woc->word[w_off].status != 0 || repl_woc->word[w_off+1].status != 0 ||
							 repl_woc->word[w_off+2].status != 0 || repl_woc->word[w_off+3].status != 0 ||
							 repl_woc->word[w_off+4].status != 0 || repl_woc->word[w_off+5].status != 0 ||
							 repl_woc->word[w_off+6].status != 0 || repl_woc->word[w_off+7].status != 0)
							occ_head_bit_8_loc[occ_head_bit_8_count++] = w_off;
						else if(repl_woc->word[w_off].status == 0)
							free_head_bit_8_loc[free_head_bit_8_count++] = w_off;
					}
				}
				
			/*	printf("\n free one word slots: ");
				for(i=0; i<free_head_bit_1_count; i++) printf("%d",free_head_bit_1_loc[i]);
				printf("\n free two word slots: ");
				for(i=0; i<free_head_bit_2_count; i++) printf("%d",free_head_bit_2_loc[i]);
				printf("\n free four word slots: ");
				for(i=0; i<free_head_bit_4_count; i++) printf("%d",free_head_bit_4_loc[i]);
				printf("\n free eight word slots: ");
				for(i=0; i<free_head_bit_8_count; i++) printf("%d",free_head_bit_8_loc[i]);
				
				printf("\n occ one word slots: ");
				for(i=0; i<occ_head_bit_1_count; i++) printf("%d",occ_head_bit_1_loc[i]);
				printf("\n occ two word slots: ");
				for(i=0; i<occ_head_bit_2_count; i++) printf("%d",occ_head_bit_2_loc[i]);
				printf("\n occ four word slots: ");
				for(i=0; i<occ_head_bit_4_count; i++) printf("%d",occ_head_bit_4_loc[i]);
				printf("\n occ eight word slots: ");
				for(i=0; i<occ_head_bit_8_count; i++) printf("%d",occ_head_bit_8_loc[i]);
				*/
				
				switch(repl_word_count)
				{
					case 0:
						repl_word_count = 0;
					break;
					case 1:
						if(free_head_bit_1_count != 0)
							repl_index = free_head_bit_1_loc[random()%free_head_bit_1_count];
						else
							repl_index = occ_head_bit_1_loc[random()%occ_head_bit_1_count];
						repl_word_count = 1;
						break;
					case 2:
						if(free_head_bit_2_count != 0)
							repl_index = free_head_bit_2_loc[random()%free_head_bit_2_count];
						else
							repl_index = occ_head_bit_2_loc[random()%occ_head_bit_2_count];
						repl_word_count = 2;
						break;
					case 3:
					case 4:
						if(free_head_bit_4_count != 0)
							repl_index = free_head_bit_4_loc[random()%free_head_bit_4_count];
						else
							repl_index = occ_head_bit_4_loc[random()%occ_head_bit_4_count];
						repl_word_count = 4;
						break;
					case 5:
					case 6:
					case 7:
					case 8:
						if(free_head_bit_8_count != 0)
							repl_index = free_head_bit_8_loc[random()%free_head_bit_8_count];
						else
							repl_index = occ_head_bit_8_loc[random()%occ_head_bit_8_count];
						repl_word_count = 8;
						break;
					default:
						repl_index = 0;
		...
		//		printf("\nRepl index: %d",repl_index);
		//		printf("\nRepl count: %d",repl_word_count);
				for(w_off = repl_index, i = 0; i<repl_word_count; w_off++, i++)
				{
					if(repl_woc->word[w_off].status & CACHE_WORD_DIRTY)
						word_dirty = 1;
					...
					/* update word tags */
					repl_woc->word[w_off].tag = repl_word_tags[i];
					if(set_head_bit == 0)
					{
						repl_woc->word[w_off].head_bit = 1;
						set_head_bit = 1;
					}
					repl_woc->word[w_off].status = CACHE_WORD_VALID;
					repl_woc->status = CACHE_BLK_VALID;
			//		printf("\n Writing Word Tag = 0x%016Lx",repl_word_tags[i]);
			//		printf(" to loc = %d",w_off);
				}
				if(repl->word_use != 0)
				{
			//		printf("\n Tag = 0x%016Lx",repl->tag);
				}
			}
  ...
  if (cp->hsize)
	...
	//Word is 32 bit, hence each word has 4 bytes
	repl->word_use = 0;
	repl->word_use = repl->word_use | (1<<(CACHE_BLK(cp, addr)/4));
...
/*
unsigned int test_cache(struct cache_t *cp)
{
 struct cache_blk_t *blk, *rehash_blk;
   md_addr_t addr;

   md_addr_t set;
   md_addr_t tag;
   unsigned int lat, count, i;
   unsigned long long int A[] = {0x00000000FBCDE120,
																		0x00000000FBCDE124,
																		0x00000000FBCFF128,
																		0x00000000FBCFF12C,
																		0x00000000FFFFE120,
   ...
   {
      addr = A[i%12];
      lat = cache_access(cp, Read, addr, NULL, 4, 10, NULL, NULL);
      printf("\nCache latency is %d", lat);
   }
}
*/
