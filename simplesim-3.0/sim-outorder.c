...
/* l1 distill cache option, i.e. <true/false> */
static char *cache_l1_distillcache_opt;
/* l1 distill cache threshold option, i.e. <true/false> */
static char *cache_l1_distillthresh_opt;

/* l2 distill cache option, i.e. <true/false> */
static char *cache_l2_distillcache_opt;
/* l2 distill cache threshold option, i.e. <true/false> */
static char *cache_l2_distillthresh_opt;
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-cache:dl1",
	...
	/* l1 distill cache option */
  opt_reg_string(odb, "-l1_distillcache",
		 "l1 distill cache flag, i.e. <true/false>}",
		 &cache_l1_distillcache_opt, "false",
		 /* print */TRUE, NULL);

	/* l1 distill cache option */
  opt_reg_string(odb, "-l1_distill_threshold",
		 "l1 distill cache threshold flag, i.e. <true/false>}",
		 &cache_l1_distillthresh_opt, "false",
		 /* print */TRUE, NULL);
		 
		/* l2 distill cache option */
  opt_reg_string(odb, "-l2_distillcache",
		 "l2 distill cache flag, i.e. <true/false>}",
		 &cache_l2_distillcache_opt, "false",
		 /* print */TRUE, NULL);

	/* distill cache option */
  opt_reg_string(odb, "-l2_distill_threshold",
		 "l2 distill cache threshold flag, i.e. <true/false>}",
		 &cache_l2_distillthresh_opt, "false",
		 /* print */TRUE, NULL); 
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	/* is the level 1 D-cache pseudo associativity enabled? */

			/* is the level 1 D-distill cache enabled? */
      if(!(mystricmp(cache_l1_distillcache_opt, "true")))
			{
				printf("L1 Distill Cache selected\n");
				distill_cache = TRUE;
			}
      ...
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			...
			if(!(mystricmp(cache_l1_distillthresh_opt, "true")) && cache_dl1->distill_cache == TRUE)
			{
				printf("L1 Distill Cache Threshold Selected\n");
				cache_dl1->threshold_distill_enabled = TRUE;
			}
/*		if(cache_dl1->distill_cache == TRUE) 
			{
				test_cache(cache_dl1);
			}
*/		
      ...
      else
	{
	  if (sscanf(cache_dl2_opt, "%[^:]:%d:%d:%d:%c",
	    ...
	    fatal("bad l2 D-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>");
		  		/* is the level 1 D-distill cache enabled? */
      if(!(mystricmp(cache_l2_distillcache_opt, "true")))
			{
				printf("L2 Distill Cache Selected\n");
				distill_cache = TRUE;
			}
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			...
			if(!(mystricmp(cache_l2_distillthresh_opt, "true")) && cache_dl2->distill_cache == TRUE)
			{
				printf("L2 Distill Cache Threshold Selected\n");
				cache_dl2->threshold_distill_enabled = TRUE;
			}
			else
				cache_dl2->threshold_distill_enabled = FALSE;
/*			if(cache_dl2->distill_cache == TRUE) 
			{
				test_cache(cache_dl2);
			}
*/ 
