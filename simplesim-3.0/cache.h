...
extern unsigned int distill_cache;
...
/* block status values */
#define CACHE_WORD_INVALID		0x00000000	/* word invalid */
#define CACHE_WORD_VALID		0x00000001	/* word in valid, in use */
#define CACHE_WORD_DIRTY		0x00000002	/* dirty word */
...
struct cache_blk_t
{
  ...
  tick_t ready;		/* time when block will be accessible, field
	...
	unsigned int word_use;
	struct cache_word_blk_t *word;
...
struct cache_word_blk_t
{
	md_addr_t tag;
	unsigned int head_bit;
	unsigned int status;
};
...
struct cache_set_t
{
  ...
  struct cache_blk_t *way_woc;	/* word oriented pf way list */
...
struct cache_t
{
  ...
  unsigned int hit_latency;	/* cache hit latency */
	unsigned int distill_cache;
	unsigned int median_threshold;
	unsigned int threshold_distill_enabled;
	unsigned int eviction_counter;
	unsigned int *word_eviction_counter;
  ...
  counter_t woc_hits;		/* total number of hits */
  counter_t hole_misses;
  counter_t line_misses;
